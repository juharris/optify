...
#[test]
fn test_cache_performance_benefit() -> Result<(), Box<dyn std::error::Error>> {
    let provider = get_new_provider();
    let cache_options = CacheOptions {};

    // This is more of a smoke test - we can't easily measure performance in unit tests
    // but we can ensure that multiple calls with caching work correctly

    let start = std::time::Instant::now();

    // Make multiple calls that should hit the cache
    for _ in 0..1000 {
        let _ = provider.get_options_with_preferences(
            "myConfig",
            &["a", "b"],
            Some(&cache_options),
            None,
    ...
    // Make calls without caching
    let start = std::time::Instant::now();
    for _ in 0..1000 {
        let _ = provider.get_options_with_preferences("myConfig", &["a", "b"], None, None)?;
    }
    let uncached_duration = start.elapsed();

    // This is just a smoke test - in a real scenario, cached should be faster
    // but we can't guarantee it in a unit test environment
    println!(
        "Cached: {:?}, Uncached: {:?}",
